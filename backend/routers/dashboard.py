...
async def get_comparison(
    ...
    """
    # Get all active competitors with their stats (filtered by tenant)
    ...
    # Calculate market average (filtered by tenant)
    ...
    # Get category breakdown (filtered by tenant)
    ...
    # Get total items count (filtered by tenant)
    ...
    # Get unacknowledged alerts count (will need tenant filter when alerts have tenant_id)
    ...
    # Get operator comparison if profile exists (filtered by tenant)
    operator_comparison = None
    op_profile_stmt = select(OperatorProfile).where(
    ...
    if op_profile:
        ...
        if op_stats and op_stats.item_count > 0:
            ...
            # Build competitor price map (filtered by tenant)
...
async def get_item_comparisons(
    ...
    """
    # Get all menu items from active competitors (filtered by tenant)
...
async def get_categories(
    ...
    """
    # Join with competitors to filter by tenant
    stmt = select(MenuItem.category).distinct().join(
        Competitor, MenuItem.competitor_id == Competitor.id
    ).where(
...
async def get_summary(
    ...
    """
    # Count competitors (filtered by tenant)
    ...
    # Count items (filtered by tenant through competitor join)
    item_count_stmt = select(func.count(MenuItem.id)).join(
        Competitor, MenuItem.competitor_id == Competitor.id
    ).where(Competitor.tenant_id == tenant_id)
    item_result = await db.execute(item_count_stmt)
    item_count = item_result.scalar() or 0

    # Count unacknowledged alerts (will need tenant filter when alerts have tenant_id)
    ...
    # Get market average (filtered by tenant)
    avg_stmt = select(func.avg(MenuItem.current_price)).join(
        Competitor, MenuItem.competitor_id == Competitor.id
    ).where(Competitor.tenant_id == tenant_id)
...
async def get_price_history(
    ...
    if limit_per_competitor is not None and not competitor_id:
        # Get all active competitors (filtered by tenant)
        comp_stmt = select(Competitor.id).where(
            Competitor.scraping_enabled == True,  # noqa: E712
            Competitor.tenant_id == tenant_id,
        )
        ...
        # Standard query with single limit (filtered by tenant)
        items_stmt = select(
            MenuItem.id,
            MenuItem.name,
            Competitor.id.label("competitor_id"),
